...
$sql = "SELECT azon, nev, ar, discounted_price FROM termekek WHERE keszlet > 0";
...
// Ha van kategória, akkor szűrés a kategória alapján
if ($category) {
    $sql .= " AND nev LIKE :category";
}

// Alapértelmezett rendezési feltétel - kezdetben nincs rendezés
$sort_order = "";

// Ha van rendezési paraméter, akkor alkalmazzuk a megfelelő rendezést
if (isset($_GET['sort']) && !empty($_GET['sort'])) {
    switch ($_GET['sort']) {
        case 'price_asc':
            $sort_order = "ORDER BY COALESCE(discounted_price, ar) ASC"; // Leárazott ár szerint, ha van
            break;
        case 'price_desc':
            $sort_order = "ORDER BY COALESCE(discounted_price, ar) DESC"; // Leárazott ár szerint, ha van
            break;
        case 'popularity':
        ...
        case 'name_desc':
            ...
            break;
        default:
            $sort_order = ""; // Ha ismeretlen érték, nincs rendezés
            break;
    }
}

// A rendezési feltételt hozzáadjuk a lekérdezéshez, ha van
...
<main class="main-content">
    <div class="sorting-container">
        ...
        <select id="sort-by" onchange="location = this.value;">
            ...
            <option value="clothes.php?sort=price_asc&search=<?= urlencode($searchTerm) ?>" <?= isset($_GET['sort']) && $_GET['sort'] == 'price_asc' ? 'selected' : '' ?>>Legolcsóbb</option>
            <option value="clothes.php?sort=price_desc&search=<?= urlencode($searchTerm) ?>" <?= isset($_GET['sort']) && $_GET['sort'] == 'price_desc' ? 'selected' : '' ?>>Legdrágább</option>
            <option value="clothes.php?sort=popularity&search=<?= urlencode($searchTerm) ?>" <?= isset($_GET['sort']) && $_GET['sort'] == 'popularity' ? 'selected' : '' ?>>Legnépszerűbb</option>
            <option value="clothes.php?sort=name_asc&search=<?= urlencode($searchTerm) ?>" <?= isset($_GET['sort']) && $_GET['sort'] == 'name_asc' ? 'selected' : '' ?>>A-Z</option>
            <option value="clothes.php?sort=name_desc&search=<?= urlencode($searchTerm) ?>" <?= isset($_GET['sort']) && $_GET['sort'] == 'name_desc' ? 'selected' : '' ?>>Z-A</option>
        </select>
    </div>

    <h1 class="products-title">Termékeink</h1> <!-- Új "Termékeink" felirat -->
